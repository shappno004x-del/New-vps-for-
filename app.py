...
def load_data():
    if not os.path.exists(DATA_FILE):
        return {"color":"#ffffff","restart":False}
...
@app.route("/toggle_restart", methods=["POST"])
def toggle_restart():
    data = load_data()
    data["restart"] = not data.get("restart", False)
    save_data(data)
    return "OK"
...
def user_panel():
    if "uid" not in session:
        return redirect("/")
    return f"User Panel: {session['uid']}"

# REAL reboot (needs sudo/root)
